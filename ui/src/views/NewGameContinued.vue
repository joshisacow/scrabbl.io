...
//   const user = inject('user');
  ...
  const startNewGame = async () => {
    const config = {
      ...
      playerNames: [userId], // Ensure username is correctly retrieved
    ...
    try {
      ...
      router.push({ name: 'LoadingScreen', params: { gameId: data.createGame, userId: userId } });
    } catch (error) {
  ...
  const joinGame = async () => {
    try {
      const playerName = userId; // This should be dynamically obtained or entered by the user
      ...
      if (data.joinGame) {
        ...
        router.push({ name: 'LoadingScreen', params: { gameId: gameId.value, userId: userId } });
      } else {
